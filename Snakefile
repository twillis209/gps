...
# Hacky and not good because chrX changes
rule fix_chrX:
     input:
      "1000g/chrX.bim"
     output:
      "1000g/chrX_fixed.bim"
     shell:
      "Rscript scripts/fix_chrX.R; cp 1000g/chrX_fixed.bim 1000g/chrX.bim"
...
rule make_plink_ranges:
     ...
     output:
      ...
      "gwas/pid_{imd}/matching_ids/chrX.txt"
     params:
      input_dir = "1000g/euro/qc",
      output_dir = "gwas/pid_{imd}/matching_ids",
     threads: 2
     shell:
      "Rscript scripts/make_plink_ranges.R -i {input.gwas_file} -b {params.input_dir} -r chr%s_qc.bim -o {params.output_dir} -nt {threads}"
...
rule cat_prune_ranges:
     input:
      ...
      "gwas/pid_{imd}/prune/chrX.prune.in"
     output:
