...
gwas_dat <- na.omit(gwas_dat, cols = c(chr_col, bp_col, 'P'))
...
for(i in seq_along(aux_cols)) {
  ...
  folds <- mclapply(unique(sub_dat[[chr_col]]),
                    function(x) sub_dat[chrom_col == x, which = T, env = list(chrom_col = chr_col)], mc.cores = no_of_threads)
  ...
  fold_indices <- folds[sapply(ind, function(x) length(x) > 0)]
  ...
  v <- mcmapply(function(x,y) vl(
                ...
                non_empty_indices, fold_indices, mc.cores = no_of_threads, SIMPLIFY = F)
  ...
  for(j in 1:length(non_empty_indices)) {
    sub_dat[non_empty_indices[[j]], (v_cols[i+1]) := il(v[[j]], pi0_null = est_q0_pars[1], sigma_null = est_q0_pars[2], distx = "norm")];
  }
