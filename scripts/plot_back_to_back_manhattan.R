...
save.image('/home/tw395/rds/hpc-work/imd_gps_pipeline/image.RData')

cfdr_dat <- fread(input_file, sep = '\t', header = T, select = c(chrom_col, bp_col, prin_col, aux_col, v_col))

cfdr_dat[!is.na(v_col), .(chrom_col, bp_col, v_col), env = list(chrom_col = chrom_col, bp_col = bp_col, v_col = v_col)] %>%
  setNames(. , c('CHR38', 'BP38', 'P')) %>%
  gwas_to_granges(., min_p = 1e-15) -> v_granges

cfdr_dat[, .(chrom_col, bp_col, prin_col), env = list(chrom_col = chrom_col, bp_col = bp_col, prin_col = prin_col)] %>%
  setNames(. , c('CHR38', 'BP38', 'P')) %>% gwas_to_granges(., min_p = 1e-15) -> prin_granges

cfdr_dat[, .(chrom_col, bp_col, aux_col), env = list(chrom_col = chrom_col, bp_col = bp_col, aux_col = aux_col)] %>%
